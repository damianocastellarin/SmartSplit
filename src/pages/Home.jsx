    ...
    <div className="space-y-6 pb-24">
      ...
      <div className="grid grid-cols-2 gap-3">
        <Link to="/create-group">
          <Button variant="secondary" className="w-full bg-white border border-slate-200 shadow-sm h-12"><Plus className="w-4 h-4 mr-2" /> Nuovo</Button>
        </Link>
        <Link to="/join-group">
          <Button variant="secondary" className="w-full bg-white border border-slate-200 shadow-sm h-12"><Search className="w-4 h-4 mr-2" /> Unisciti</Button>
        </Link>
      </div>
        ...
        {groups.length === 0 ? (
          <Card className="border-dashed border-2 bg-slate-50">
            <CardContent className="flex flex-col items-center justify-center py-12 text-center">
              <div className="bg-blue-100 p-4 rounded-full mb-4"><Users className="w-8 h-8 text-primary" /></div>
              <h3 className="font-semibold text-lg mb-2">Nessun gruppo</h3>
              <p className="text-slate-500 text-sm mb-6 max-w-[250px]">Crea un nuovo gruppo o unisciti a uno esistente tramite codice.</p>
            </CardContent>
          </Card>
        ) : (
          groups.map((group) => {
                    ...
                    <div className="flex items-center gap-3">
                      {isEmpty ? (
                        <div className="flex items-center gap-1.5 bg-slate-100 text-slate-600 px-3 py-1.5 rounded-full text-xs font-bold shadow-sm border border-slate-200">
                          <Sparkles className="w-3.5 h-3.5" /> <span className="hidden sm:inline">Nuovo</span>
                        </div>
                      ) : (
                        <div className="flex items-center gap-1.5 bg-blue-50 text-blue-700 px-3 py-1.5 rounded-full text-xs font-bold shadow-sm border border-blue-100">
                          <Receipt className="w-3.5 h-3.5 opacity-70" /> <span>{totalGroupSpent.toFixed(2)} €</span>
                        </div>
                      )}
